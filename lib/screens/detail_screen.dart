        ...
        children: [
          Consumer<FavoriteProvider>(
            builder: (context, favorite, child) {
              final liked = favorite.isFavorite(widget.monAn.id);
              return IconButton(
                tooltip: liked ? 'Bỏ yêu thích' : 'Thêm vào yêu thích',
                icon: Icon(
                ...
                onPressed: () async {
                  await favorite.toggleFavorite(
                    FavoriteItem.fromMonAn(widget.monAn),
                  );
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        liked
                            ? 'Đã bỏ ${widget.monAn.tenMon} khỏi yêu thích'
                            : 'Đã thêm ${widget.monAn.tenMon} vào yêu thích',
                      ),
